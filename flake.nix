      ...
      perSystem =
        ...
        }:
        {
          ...
          packages.service = pkgs.runCommand "pass-secret-service" { } ''
            ...
            sed -e "s|/usr/bin/pass-secret-service|${lib.getExe self'.packages.default}|g" "${./systemd/org.freedesktop.secrets.service}" > "$out/share/dbus-1/services/org.freedesktop.secrets.service"
            sed -e "s|/usr/bin/pass-secret-service|${lib.getExe self'.packages.default}|g" "${./systemd/pass-secret-service.service}" > "$out/lib/systemd/user/pass-secret-service.service"
          '';
